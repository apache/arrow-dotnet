...
namespace Apache.Arrow.Serialization.Generator
{
    ...
    public class ArrowSerializerGenerator : IIncrementalGenerator
    {
        ...
        {
            // Generated registrations use [ModuleInitializer]; the attribute is missing
            // from the BCL below .NET 5, so emit an internal polyfill when needed.
            // Referenced assemblies may carry their own internal polyfill (e.g.
            // Microsoft.CodeAnalysis), which GetTypeByMetadataName would find even though
            // the consumer compilation cannot use it — require an accessible declaration.
            var needsModuleInitializerPolyfill = context.CompilationProvider.Select(static (compilation, _) =>
            {
                foreach (var type in compilation.GetTypesByMetadataName("System.Runtime.CompilerServices.ModuleInitializerAttribute"))
                {
            ...
            context.RegisterSourceOutput(needsModuleInitializerPolyfill, static (spc, needed) =>
            {
                if (!needed)
                    return;
                spc.AddSource("ModuleInitializerAttribute.Polyfill.g.cs",
                    "// <auto-generated/>\n" +
